...
df_results = pd.read_pickle("/home/user/Documents/Programmation/Eve/Tests_NoteTech_Pierre/results/Donnees_Pierre/Dataframe_results_metrics.pkl")

df_results["Exp_Imp_Imp_jerk"] = None
explicit_bool = np.logical_or(df_results["dynamics_type"] == MillerDynamics.EXPLICIT,
                              df_results["dynamics_type"] == MillerDynamics.ROOT_EXPLICIT)
df_results.loc[explicit_bool, "Exp_Imp_Imp_jerk"] = "EXPLICIT"
implicit_bool = np.logical_or(df_results["dynamics_type"] == MillerDynamics.IMPLICIT,
                              df_results["dynamics_type"] == MillerDynamics.ROOT_IMPLICIT)
df_results.loc[implicit_bool, "Exp_Imp_Imp_jerk"] = "IMPLICIT"
implicit_qdddot_bool = np.logical_or(df_results["dynamics_type"] == MillerDynamics.IMPLICIT_TAU_DRIVEN_QDDDOT,
...
full_bool = np.logical_or(np.logical_or(df_results["dynamics_type"] == MillerDynamics.EXPLICIT,
                          df_results["dynamics_type"] == MillerDynamics.IMPLICIT),
                          df_results["dynamics_type"] == MillerDynamics.IMPLICIT_TAU_DRIVEN_QDDDOT)
df_results.loc[full_bool, "Root_Full"] = "FULL_BODY"
root_bool = np.logical_or(np.logical_or(df_results["dynamics_type"] == MillerDynamics.ROOT_EXPLICIT,
                          df_results["dynamics_type"] ==MillerDynamics.ROOT_IMPLICIT),
                          df_results["dynamics_type"] == MillerDynamics.ROOT_IMPLICIT_QDDDOT)
...
def function_test_anova(df_results, to_be_tested):
    model = ols(f'{to_be_tested} ~ C(Exp_Imp_Imp_jerk) + C(Root_Full) + C(Exp_Imp_Imp_jerk):C(Root_Full)', data=df_results).fit()
