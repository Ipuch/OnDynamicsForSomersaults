...
def root_explicit_dynamic(
    ...
    non_linear_effects = nlp.model.NonLinearEffect(q, qdot).to_mx()

    # qddot_root = -M_BB^-1 * ( M_BJ  * qddot_joints + N_B )
    qddot_root = -mass_matrix_inverse[:nb_root, :nb_root] @ mass_matrix_nl_effects

    # qddot_root = -mass_matrix_inverse[:nb_root, :nb_root] @ (mass_matrix[:nb_root, nb_root:] @ qddot_joints + non_linear_effects[:nb_root])

    return qdot, cas.vertcat(qddot_root, qddot_joints), qdddot_joints
...
def custom_configure_root_explicit(ocp: OptimalControlProgram, nlp: NonLinearProgram):
    ...
    ConfigureProblem.configure_qddot(nlp, as_states=True, as_controls=False)
    ConfigureProblem.configure_qdddot(nlp, as_states=False, as_controls=True)
